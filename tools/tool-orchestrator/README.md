...
The Cockpit home surface is a local-first widget dashboard. It ships a default layout without writing workspace files. Its wide `Subscription limits` widget shows remaining 5-hour and weekly quota for Codex and Claude Code (see below). Its wide `Needs attention` widget leads with the selected-workspace Operator count and up to three linked Operator tasks, with the Agent-ready count as secondary context. A calm zero state means no actionable task currently names the Operator; Task Browser owns inclusion and ordering through its summary API.
...
- **Codex · Weekly** and **Codex · 5 hour** — read from the Codex CLI app-server
  method `account/rateLimits/read`. The call is retrieval-only: no thread and no
  turn are started, so no model runs. Buckets are selected by
  `windowDurationMins` (`10080` and `300`) rather than by primary/secondary
  position. Both gauges come from one app-server response.
...
The two weekly gauges share the first row, with each provider's five-hour gauge
below its weekly one; narrow layouts stack all four.
