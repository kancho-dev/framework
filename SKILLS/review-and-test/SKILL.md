...
8. For meaningful Oracle task reviews, keep the review as a separate Oracle task run when possible. If write-capable, record a run log, update task state, append a timed entry to the end of today's daily brief, and align task-browser metadata when the workspace uses task-browser unless the review is explicitly read-only/no-write. Typical status outcomes are `done` after approval/closure, `active` after a bounce, and `review` when review is still pending. Set `nextActor` to `agent` for Oracle review or a Builder bounce, `operator` for Operator review/acceptance, and `null` after closure or when no concrete action remains, following the authoritative matrix in `framework/TASKS.md`; keep it aligned with `HANDOFF.md`, not inferred from status. In framework versions with task-browser action history, pass real provenance such as `--role Oracle` and actual session details when useful and available; leave provenance unset rather than inventing it.
